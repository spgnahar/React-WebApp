...
import ReactTooltip from 'react-tooltip'
 ...
 class Business extends React.Component{
	...
	componentDidMount(){
        var url="https://newsapp-backend.appspot.com/guardian/section?sec=business";
        ...
        var url1="https://newsapp-backend.appspot.com/newyork/section?sec=business";
		fetch(url1)
		.then(response => {
			if(response.status !==200){
				throw new Error("File not recieved")
    ...
    handleSearchChange = async (inputValue ) => {
	  ...
	  try {
		const response = await fetch(
		  ...
		  {
			headers: {
			  "Ocp-Apim-Subscription-Key": "67958d8145bc4e49bdbc2d248cebf955"
			}
		  }
		);
	...
	render(){
		...
		<Navbar.Collapse id="responsive-navbar-nav">
			<Nav className="mr-auto" >
				...
				<Nav.Link as={Link} to="/Business" href="/Business" style={{color:"white"}}>Business</Nav.Link>
